 ...
 * src/middleware/security.js (Basic Auth do app Express) e do ApiKeyGuard
 * já usado em services/crm.
 *
...
export class ApiKeyGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    const expected = process.env.FINANCEIRO_SERVICE_API_KEY;
    if (!expected) {
      throw new UnauthorizedException('FINANCEIRO_SERVICE_API_KEY não configurado no servidor.');
    }
